##########################################################
# Analyze coding functional categories using SurvSTAAR
...
for (j in 1:length(idSet)) {
  ...
  geneCoding_results[[j]] = try(geneCentricCoding(gene_name = gene_name,
                                                  genofile = genofile,
                                                  objNull = objNull,
  ...
  save(geneCoding_results, file = output_name)
