...
for chr in {1..22}; do
    ...
    for set in $(seq 1 $Nsets); do
        echo "${chr} ${set} ${Nsets} NonCoding_results_chr${chr}_${set} objNull_chr${chr}.rda ukb.500k.wgs.chr${chr}.pass.annotated.gds all Annotation_name_catalog.rda annotation/info/QC_label TRUE"
    done
done  >> args_NonCoding.txt
